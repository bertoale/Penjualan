<template>
  <div class="max-w-6xl mx-auto">
    ...
    <div v-if="loading" class="text-center py-8">
      <div class="text-gray-500">Memuat data penjualan...</div>
    </div>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
          ...
          <button
            ...
            class="w-full mt-6 bg-green-600 text-white py-3 px-4 rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
          >
            {{ isProcessing ? "Memproses..." : "Update Penjualan" }}
          </button>
        </div>
      </div>
    </div>
      ...
      >
        <div class="mt-3 text-center">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
            Transaksi penjualan telah berhasil diperbarui.
          </p>
...
const penjualanId = ref(route.params.id);
...
const fetchSaleData = async () => {
  try {
    loading.value = true;
    const token = localStorage.getItem("token");
    const response = await axios.get(`${url}/penjualan/${penjualanId.value}`, {
      headers: { Authorization: `Bearer ${token}` },
    });

    const saleData = response.data.penjualan;

    // Convert sale details to cart items
    cartItems.value = saleData.PenjualanDetails.map((detail) => ({
      id: detail.Produk.id,
      nama: detail.Produk.nama,
      harga_jual: parseFloat(detail.harga_jual),
      quantity: detail.qty,
      stok: detail.Produk.stok + detail.qty, // Add back the quantity to get original stock
      ProdukKategori: detail.Produk.ProdukKategori,
    }));

    saleDate.value = new Date(saleData.tanggal).toISOString().split("T")[0];
  } catch (err) {
...
const updateSale = async () => {
  ...
  try {
    ...
    await axios.put(`${url}/penjualan/${penjualanId.value}`, saleData, {
      headers: {
